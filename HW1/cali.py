...
# Load image
img = cv2.imread("middle_finger.jpg")   # Replace with your image
img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

# Apply barrel (k1 < 0) and pincushion (k1 > 0)
barrel = apply_radial_distortion(img, k1=-0.5)
pincushion = apply_radial_distortion(img, k1=0.5)

# Show results
plt.figure(figsize=(12,6))
plt.subplot(1,3,1), plt.imshow(img), plt.title("Original")
plt.subplot(1,3,2), plt.imshow(barrel), plt.title("Barrel (k1=-0.3)")
plt.subplot(1,3,3), plt.imshow(pincushion), plt.title("Pincushion (k1=0.3)")
plt.show()
